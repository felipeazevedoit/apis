...
namespace SaudeComVc_Home.Models
{
    public class PerguntaViewModel : BaseModel
    {
        ...
        [JsonIgnore]
        public IDictionary<PerguntaViewModel, PerguntaViewModel> PerguntasFilhos { get; set; }

        public PerguntaViewModel()
        {
            TipoResposta = new TipoRespostaViewModel();
            Respostas = new List<RespostaViewModel>();
            PerguntasFilho = new List<PerguntaViewModel>();
        }
    }
}