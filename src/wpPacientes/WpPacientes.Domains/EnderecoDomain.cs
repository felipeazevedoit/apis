    ...
    public class EnderecoDomain : IDomain<Endereco>
    {
        ...
        public async Task DeleteAsync(Endereco entity, string token)
        {
            try
            {
                ...
                var endereco = _edRepository.GetList(e => e.PacienteId.Equals(entity.PacienteId)).SingleOrDefault();
                endereco.Status = 9;
        ...
        public IEnumerable<Endereco> GetAlls(IEnumerable<int> pacienteIds, string token)
        {
            try
            {
                //await _segService.ValidateTokenAsync(token);
                var enderecos = _edRepository.GetList(e => pacienteIds.Contains(e.PacienteId));
                return enderecos;
            }
            catch (ServiceException e)
            {
        ...
        public async Task<Endereco> GetByPacienteIdAsync(int pacienteId, int idCliente, string token)
        {
            try
            {
                await _segService.ValidateTokenAsync(token);

                var endereco = _edRepository.GetList(e => e.PacienteId.Equals(pacienteId)
                                        && e.IdCliente.Equals(idCliente)).SingleOrDefault();
                return endereco;
            }
            catch (ServiceException e)
        ...
        public async Task DeleteAsync(int pacienteId, string token)
        {
            try
            {
                await _segService.ValidateTokenAsync(token);
                var endereco = _edRepository.GetList(e => e.PacienteId.Equals(pacienteId)).SingleOrDefault();
